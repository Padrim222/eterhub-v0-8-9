...
import type { Theme } from "@/hooks/useEterflow";
...
const getFormatBadge = (format: string) => {
  ...
  if (lower.includes("reel") || lower.includes("video")) {
    return { label: "🎬 Reels", className: "bg-purple-500/20 text-purple-400 border-purple-500/50" };
  }
  if (lower.includes("carousel") || lower.includes("carrossel")) {
    return { label: "📚 Carrossel", className: "bg-blue-500/20 text-blue-400 border-blue-500/50" };
  }
  return { label: "🖼️ Post", className: "bg-green-500/20 text-green-400 border-green-500/50" };
};

export function ThemesSelector({ themes, onSelect, selectedIndex, isLoading }: ThemesSelectorProps) {
  if (themes.length === 0) {
    return (
      <Card className="bg-gray-800/50 backdrop-blur-sm border-gray-700/50 p-8 text-center">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <Badge variant="outline" className={`text-xs ${formatBadge.className}`}>
                        {formatBadge.label}
                      </Badge>
                      {selectedIndex === index && (
